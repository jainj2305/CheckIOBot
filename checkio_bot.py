...
def read_credentials():
    secrets = 'credentials.json'          #Update credentials.json file with your username and password of CheckIO.
...
class CheckIOSolver:
    def __init__(self, loginID, password):
        ...
        self.driver=webdriver.Chrome(executable_path="F:\setup\chrome driver\chromedriver\chromedriver.exe") #Path to chrome drivers
        self.COMMAND_OR_CONTROL = Keys.COMMAND if sys.platform == 'darwin' else Keys.CONTROL  #for windows or mac
    ...
    def put_credentials_to_form(self):
        try:
            self.driver.find_element_by_id("id_username").send_keys(self.loginID)
            time.sleep(2)
...
def main():
    ...
    bot = CheckIOSolver(credentials['username'], credentials['password'])  #Update credentials.json file with your username and password of CheckIO.
